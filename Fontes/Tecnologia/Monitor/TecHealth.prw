...
//-------------------------------------------------------------------
/*/{Protheus.doc} tTecHealth
Definição da Classe: Classe de Exposição de valores de Equipe de Tecnologia TOTVS
...
/*/
//-------------------------------------------------------------------
Class tTecHealth //From tRedisClient
Data   cKVSrv
Data   nKVPort
Data   KVOnConnected
Data   cCmdKVRun
...
If ::connect(cKVSrv,nKVPort):ok()
    ::KVOnConnected := .T.
Else
    ConOut(STR0001) //"KV ConKV, verifique os dados ou avise ao Administrador do ambiente."
    ::lHasConnError := .T.
    ::cHasConnError := AllTrim(Str(::nError)) + "|" + ::AllTrim(::cError)    // msg original completa
EndIf
