...
If cTipoAtf $ "01#03"
	...
	For nI := 1 to oModelFNG:Length()
		...
		If nI != nLinha .And. oModelFNG:GetValue("FNG_TIPO") == cTipoAtf .And. !oModelFNG:IsDeleted()
			Help(" ",1,"TS271TIP0",,STR0003,1,0)  //"É permitido a inclusão de apenas um item do tipo 01/03 por grupo"
		    lRet := .F.
		    Exit
		EndIf
	Next nI
	...
	If cTipoAtf $ "01#10#16#17" 
		...
		For nI := 1 to oModelFNG:Length()
			...
			If nI != nLinha .And. oModelFNG:GetValue("FNG_TIPO") $ "03#13" .And. !oModelFNG:IsDeleted()
				Help(" ",1,"TS271TPAQ",,STR0015,1,0)  //"Não é permitido a configuração de ativos de aquisição com ativos de adiantamento."
				lRet := .F.
				Exit
			EndIf
		Next nI
	...
	If cTipoAtf $ "03#13"
		...
		For nI := 1 to oModelFNG:Length()
			...
			If nI != nLinha .And. oModelFNG:GetValue("FNG_TIPO") $ "01#10#16#17" .And. !oModelFNG:IsDeleted()
				Help(" ",1,"TS271TPAD",,STR0016,1,0)  //"Não é permitido a configuração de ativos de adiantamento com ativos de aquisição."
				lRet := .F.
				Exit
			EndIf
		Next nI
	...
	If lArgentina
		If Alltrim(oModelFNG:GetValue("FNG_CRIDEP")) $ "03|04"
			If !(oModelFNG:GetValue("FNG_TIPO") $ "10|12|16|17")
				Help(" ",1,"TS271VDEPC",,STR0012,1,0)//"Critério de depreciação não é valido para o tipo de ativo em questão"
				lRet:= .F.
			EndIf
		Endif
	EndIf
...
If lExistAud //  Retorno da função
	lRet := .F.
	Help(" ",1,"TS271IMPNAO",,STR0039 ,1,0)//"Esse grupo já existe."
EndIf
...
If cPatrim == "T" .AND. !oAuxBusca:GetValue('FNG_TIPO',nLinha) $ ('01|10' + cTypes10) .AND. !oAuxBusca:IsDeleted(nLinha)
	Help( " ",1,"TS012PERM" ) // "Não é permitido usar o tipo nesta operação!"
	lRet := .F.
EndIf
...
If lRet .And. !lLinOk .and. cTipo == "14" .And. !oAuxBusca:IsDeleted() //Linha nova
	Help( " ", 1, "TS012NOT14")	//"Não é permitida a inclusão manual de Tipo 14. Utilize o processo de AVP padrão."
	lRet := .F.
EndIf

//MRG
If lRet .And. !lLinOk .and. cTipo == "15" .And. !oAuxBusca:IsDeleted() //Linha nova
	Help( " ", 1, "TS012NOT15",,)	//"Não é permitida a inclusão manual de Tipo 15. Utilize o processo de Margem Gerencial padrão."
	lRet := .F.
EndIf
...
If lRet .AND. aScan(aTiposBase,{|x| cTipo == x}) == 0 .and. !Empty(cTipo) 
	...
	If Empty(nTipoUnico)
		Help( " ", 1, "TS012TIP",, STR0053, 1, 0 ) //"Tipo de ativo não é um tipo base"
		lRet := .F.
	EndIf
...
If lRet .and. cPatrim = 'E' .and. !(cTipo $ "03/13/10"+cTypes10) .And. !oAuxBusca:IsDeleted()
	Help( " ", 1, "TS012EMP") //""Só permitidos os tipos 03 ou 13 quando o Bem for classificado como Custo de Emprestimo"
	lRet := .F.
Endif
